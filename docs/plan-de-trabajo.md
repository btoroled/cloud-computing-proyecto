...
|---|---|---|
| `cloud-computing-proyecto` (este) | Toda la documentación: `docs/`, `diagramas/`, `informe/`, `ppt/`, `INDEX.md` | Benja |
| `ms1-pasajeros-api` | MS1 Python/FastAPI/MySQL + `Dockerfile` + `compose` local + migraciones + `openapi.yaml` + tests | Guillermo |
| `ms2-vuelos-api` | MS2 Java/Spring Boot/PostgreSQL + ídem | Mariano |
| `ms3-infraestructura-api` | MS3 Node/Express/MongoDB + ídem | Edinson |
| `ms4-manifiesto-api` | MS4 Python/FastAPI (sin BD) + ídem | Fabricio |
| `ms5-analitica-api` | MS5 Python/FastAPI + boto3 + SQL de las consultas Athena | Fabricio |
| `aeropuerto-frontend` | SPA React + config Amplify + página Swagger-UI agregada | Alexander |
| `aeropuerto-data-science` | 3 contenedores de ingesta + `seeds/` (generador de data ficticia) + scripts Glue + DDL de vistas Athena | Fabricio + Guillermo/Mariano/Edinson |
...
|---|---|---|
| **Líder / Arquitecto** | Benja | `aeropuerto-infra-deploy` completo · VPC/subredes/SG · 4 EC2 · ALB interno · API Gateway + VPC Link · nginx gateway · despliegue `compose` en las 3 VMs · diagrama `draw.io` de arquitectura · pruebas de integración E2E · control del camino crítico · consolidación del informe |
| **Guillermo** | — | `ms1-pasajeros-api` · E/R MySQL · llamada MS1→MS2 · contenedor `ingesta-ms1` · seed 20k `ticket`/`equipaje` |
| **Mariano** | — | `ms2-vuelos-api` · E/R PostgreSQL · endpoint `/vuelos/{id}/exists` · contenedor `ingesta-ms2` · seed 20k `vuelo`/`asiento` |
| **Edinson** | — | `ms3-infraestructura-api` · JSON schemas · llamada MS3→MS2 · contenedor `ingesta-ms3` (con aplanado) · seed 20k `incidencias` |
| **Fabricio** | — | `ms4-manifiesto-api` · `ms5-analitica-api` · catálogo Glue · ≥4 consultas Athena + ≥2 vistas · E/R del data lake · `seeds/` compartido |
| **Alexander** | — | `aeropuerto-frontend` (4 vistas) · deploy Amplify + contingencia · página Swagger-UI agregada · apoyo a Fabricio en gráficos del dashboard |

Ceremonias: **standup diario de 15 min** con Benja (bloqueos + camino crítico); revisión de contratos
...
- **Guillermo/Mariano/Edinson:** `docker compose up` local del servicio + su BD, con migraciones y 1 endpoint real.
- **Fabricio:** estructura de `aeropuerto-data-science`; escribir las 5 consultas de negocio en SQL sobre el
  modelo relacional (validarlas en Postgres local antes de Athena).
- **Alexander:** scaffold React desplegado (Amplify o contingencia) con 1 llamada real a MS2 (`GET /vuelos`).
...
- **Guillermo:** CRUD de pasajeros/tickets/checkin/equipaje; llamada MS1→MS2 al emitir ticket; seed ~5k.
- **Mariano:** CRUD de vuelos/aerolíneas/aeronaves/asientos/tripulación; `/vuelos/{id}/exists`; seed ~2k vuelos.
- **Edinson:** CRUD de recursos/incidencias/asignaciones; llamada MS3→MS2; seed ~5k incidencias.
- **Fabricio:** `GET /manifiesto/{vuelo_id}` consumiendo Guillermo/Mariano/Edinson; **VM-INGESTA + bucket S3 + contenedor
  `ingesta-ms1`** cargando datos reales a S3.
- **Alexander:** frontend desplegado consumiendo **≥1 microservicio con ≥2 métodos REST**.
- **Benja:** nginx gateway en las 2 VM-PROD; `compose` con los 5 servicios; ALB interno; API Gateway +
...
- **Guillermo/Mariano/Edinson:** completar endpoints; **carga masiva ≥20 000 registros por BD (una vez)**; validación de
  integridad cruzada; Swagger completo.
- **Edinson:** aplanado de `incidencias` en el contenedor de ingesta.
- **Fabricio:** los **3 contenedores de ingesta** corriendo (pull 100%) → S3; **crawlers Glue** → catálogo;
  **≥4 consultas Athena con join + ≥2 vistas**; MS5 exponiendo las 5 consultas; **E/R del data lake**.
- **Alexander:** **4 vistas** del frontend consumiendo **los 5 microservicios** (≥2 métodos REST c/u), incluida
  la vista de analítica (MS5); página Swagger-UI agregada; deploy final en Amplify.
- **Benja:** **diagrama de arquitectura `draw.io`** con todos los servicios AWS; **pruebas E2E** por la
...
1. **Benja:** crear la organización GitHub + los 9 repos (README + LICENSE + plantilla) e invitar a los 5 devs.
2. **Benja:** validar en Learner Lab — EC2+VPC → S3 → Glue+Athena → API Gateway+VPC Link → Amplify — y
...
4. **Guillermo/Mariano/Edinson:** `docker compose up` local del servicio + su BD, con migraciones y 1 endpoint real.
5. **Fabricio:** escribir las 5 consultas de negocio en SQL sobre el modelo relacional (validar en Postgres local).
6. **Alexander:** scaffold del frontend desplegado con una llamada real a `GET /api/vuelos`.
